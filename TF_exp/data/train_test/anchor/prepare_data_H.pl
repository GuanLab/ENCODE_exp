...
$path1="/state3/hyangl/TF_model/data/train_test/macs/H/"; # PARAMETER: change the directory names F,G,H,I
$path2="/state3/hyangl/TF_model/data/sample/03/"; # PARAMETER: F-01,G-02,H-03,I-04
...
while ($tf=<FILE>){
    ...
    foreach $cell (@list){
        ...
        foreach $file (@tf_feature){
            $name="INPUT".$num;
            open $name, "${file}" or die; # PARAMETER: FG ${file}${tf} HI ${file}
            $num++;
        }
